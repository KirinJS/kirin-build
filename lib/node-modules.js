...
NodeModule.prototype.discoverAll = function (mockPackageJson, seen) {
    ...
    function getPackageDependencies() {
        // we need to take into consideration optional dependencies here.
        // we want to be able to specify dependencies for the javascript that is run on the device, 
        // though we may have things that are necessary for building. 
        // For ease of installation, I'm not sure how to best use devDependencies, (it installs/compiles
        // devDependencies for all dependencies, not just the one we're npm installing), so 
        // we're using optionalDependencies, which gets installed by default.
        // Unfortunately, the npm publish command seems to smoosh the dependencies and optionalDependencies 
        // all together into the dependencies block, which completely defeats the point of using optionalDependencies. 
        // So we're going to take the difference between dependencies and optionalDependencies, assuming that you haven't 
        // specified dependencies that you actually need on the device in optionalDependencies redundantly.
        var dep = _.keys(packageJson.dependencies || {}),
            optDep = _.keys(packageJson.optionalDependencies || {});
        return _.difference(dep, optDep);
    }
    
    // now let's look at dependencies
    // If we really get ourselves in a mess then we should fall back to the kirinInfo.dependencies.
    // TODO I'm not sure how useful this is now.
    var dependencies = kirinInfo.dependencies || getPackageDependencies();
