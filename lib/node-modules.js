    ...
    _.each(dependencies, function (package_) {
        ...
        if (!main && package_ === 'kirin-core') {
            ...
            if (!fsHelper.findFileSuffix(main, "package.json")) {
                console.error("Panic!");
                main = null;
                throw new Error("Can't find kirin-core. You can set the environment KIRIN_HOME if you're not on the node search path.");
            }
            
        } else if (!main) {
            err = err || "Cannot find a package.json file for " + package_ + " (from "+ self.packageRoot + ")";
            throw new Error(err);
        }
        
        var newModule = exports.module(main, package_);
...
exports.module = function (path, mockPackageJson) {
    ...
    if (_.isString(mockPackageJson)) {
        m.name = mockPackageJson;
