...
Fileset.prototype.crawl = function (includeEverything) {
    var self = this,
    ...
    _.each(files, function (filepath) {
        ...
        if (!includeEverything) {            
            if (relative.indexOf("node_modules") >= 0 || relative.indexOf(".git") >= 0 || relative.indexOf(".svn") >= 0) {
                return;
            }
        }
        ...
        if (includeEverything || (!existing || fileObject.specificity > existing.specificity)) {            
    ...
    _.each(fileMap, function (i, canonical) {
        if (includeEverything || fileMap[canonical].specificity >= 0) {
