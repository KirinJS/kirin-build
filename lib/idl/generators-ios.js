    ...
    IOS.prototype.methodLine = function (sig) {
        ...
        if (numParams === 0) {
            ...
            for (i=0; i<numParams; i++) {
                ...
                var typeName = typeString(p.type),
                    paramName = p.name;
                console.log("About to generate method param for " + typeName + " " + paramName);
                if (this.usage === "native" && !_.isString(p.type)) {
                    //     [self.kirinHelper proxyForJavascriptRequest:@protocol[(blah) andDictionary:object]
