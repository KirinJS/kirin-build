...
Translator.prototype.translate = function (nodeModule, writeToDisk) {
    ...
    _.each([argv.platform, "javascript"], function (suffix) {
        var generator;
        ...
        if (writeToDisk) {
            var filepath = generator.getOutputDirectory(kirinInfo, nodeModule.platformBlock);
            if (!argv.dryRun) {
                fsHelper.rmRf(filepath);
            } else {
        ...
        _.each(irSymbolTables, function (irSymbolTable) {
            generator.generateFiles(kirinInfo, nodeModule.platformBlock, irSymbolTable, targetFiles);
        });
    });
