...
exports._lintPackage = function lintPackage (package_, options) {
    ...
    if (!options || !_.isObject(options)) {
        ...
        if (argv.all || argv.lintAll) {
            ...
            var libFile = path.resolve(package_.directory, ".jshintrc");
            var rootFile = path.resolve(package_.packageRoot, ".jshintrc");
            ...
            if (!fs.existsSync(configFile)) {
                // console.warn("# Can't find " + rootFile + " file. Skipping jshint");
                return;
            }
        }
